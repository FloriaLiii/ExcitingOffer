...
Page({
  ...
  statr_sc(e) {
    ...
    if (openid) {
      ...
      let data = this.data.salary_list
...
//岗位
  gw: function (id) {
    var that = this
    db.collection('salary').where({
      _id: id
    }).get().then(res => {
      let result = res.data[0].html
      that.setData({
        html: result,
        salary_list: res.data[0]
      })
      console.log(this.data.salary_list.img)
    })
  ...
  //面试
  fy() {
    db.collection('salary').skip(this.data.limt + 1).limit(1).get().then(res => {
